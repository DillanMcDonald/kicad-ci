...
traffic is made.  Mock responses match the actual JSON schemas documented by
each distributor's API reference.
"""
...
class TestMouserClient:
    ...
    def test_price_breaks_use_decimal(self, tmp_path):
        ...
        for pb in result.price_breaks:
            assert isinstance(pb.unit_price_usd, Decimal), (
                f"price_break.unit_price_usd should be Decimal, got {type(pb.unit_price_usd)}"
            )
    ...
    def test_infinity_tier_uses_prev_qty_plus_one(self, tmp_path):
        ...
        # Mock: tiers 100,500,1000 then Infinity -> 1001
...
class TestNexarClient:
    ...
    def test_search_by_mpn_returns_best(self, tmp_path):
        ...
        assert result is not None
        # Mouser has lower price at qty=100 (0.0128 vs DigiKey 0.013)
        assert result.distributor == "mouser"
    ...
    def test_7_day_cache_ttl(self, tmp_path):
        ...
        assert _CACHE_TTL_HOURS == 168.0, "Nexar cache TTL should be 7 days (168h)"
    ...
    def test_empty_hits_returns_empty(self, tmp_path):
        ...
        with patch.object(client._session, "post", return_value=mock_resp):
            results = client.search_by_mpn_multi("NO-SUCH-PART-XYZ")
...
class TestPriceResult:
    ...
    def test_price_at_qty_below_first_break(self):
        ...
        # qty < first break → no applicable tier
...
# ---------------------------------------------------------------------------
# BomLine aggregation tests
# ---------------------------------------------------------------------------

class TestBomCsvReader:
    def _write_csv(self, tmp_path, rows, headers=None):
        p = tmp_path / "bom.csv"
        default_headers = ["Reference", "MPN", "Manufacturer", "Quantity", "Value", "Footprint", "Description", "DNP"]
        h = headers or default_headers
        with p.open("w", newline="") as fh:
            import csv as _csv
            writer = _csv.DictWriter(fh, fieldnames=h)
            writer.writeheader()
            for row in rows:
                writer.writerow(row)
        return p
    ...
    def test_basic_parse(self, tmp_path):
        from scripts.pricing_xlsx import read_bom_csv
        p = self._write_csv(tmp_path, [
            {"Reference": "R1", "MPN": "RC0402FR-07100KL", "Manufacturer": "Yageo",
             "Quantity": "1", "Value": "100k", "Footprint": "0402", "Description": "Resistor", "DNP": ""},
        ])
    ...
    def test_dnp_excluded(self, tmp_path):
        from scripts.pricing_xlsx import read_bom_csv
        p = self._write_csv(tmp_path, [
            {"Reference": "R1", "MPN": "PART-A", "Manufacturer": "X",
             "Quantity": "1", "Value": "10k", "Footprint": "0402", "Description": "", "DNP": ""},
            {"Reference": "R2", "MPN": "PART-B", "Manufacturer": "X",
             "Quantity": "1", "Value": "10k", "Footprint": "0402", "Description": "", "DNP": "1"},
        ])
    ...
    def test_same_mpn_grouped(self, tmp_path):
        ...
        p = self._write_csv(tmp_path, [
            {"Reference": "R1", "MPN": "RC0402FR-07100KL", "Manufacturer": "Yageo",
             "Quantity": "1", "Value": "100k", "Footprint": "0402", "Description": "", "DNP": ""},
            {"Reference": "R2", "MPN": "RC0402FR-07100KL", "Manufacturer": "Yageo",
             "Quantity": "1", "Value": "100k", "Footprint": "0402", "Description": "", "DNP": ""},
        ])
        lines = read_bom_csv(p)
        assert len(lines) == 1
    ...
    def test_blank_mpn_skipped(self, tmp_path):
        from scripts.pricing_xlsx import read_bom_csv
        p = self._write_csv(tmp_path, [
            {"Reference": "R1", "MPN": "", "Manufacturer": "X",
             "Quantity": "1", "Value": "?", "Footprint": "0402", "Description": "", "DNP": ""},
            {"Reference": "C1", "MPN": "GRM155R61A106KE19D", "Manufacturer": "Murata",
             "Quantity": "2", "Value": "10uF", "Footprint": "0402", "Description": "Cap", "DNP": ""},
        ])
        lines = read_bom_csv(p)
        assert len(lines) == 1
